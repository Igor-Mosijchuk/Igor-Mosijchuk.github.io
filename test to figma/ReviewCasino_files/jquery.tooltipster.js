...
$(document).ready(function(){
	...
	function panel_search() {
		var flag = 0;
		$('#main-menu .panel-search').on('click', function (e) {
			e.stopPropagation();
			if (flag == 0) {
				$('#main-menu .nav-search').css('width','45%');
				$('.search-log.panel-search').addClass('active');
				$('.search-close, #searchResult').css('display','block');
				$('.search-close').css('z-index','99');
				$('#search_fd').focus();
				flag = 1;
				return false;
			};
		...
		$(document).on('click', '.search-close', function (e){
			if( $(window).width() > 1024 && flag == 1){
				$('#main-menu .nav-search').css('width','62px');
				$('.search-log.panel-search').removeClass('active');
				$('.search-close, #searchResult').css('display','none');
				$('.search-close').css('z-index','50');
				flag = 0;
				} else{
				$('#main-menu .nav-search').css('width','45px');
				$('.search-log.panel-search').removeClass('active');
				$('.search-close, #searchResult').css('display','none');
				$('.search-close').css('z-index','50');
				flag = 0;
	...
	$('#loginToggleForm input[type="text"]').tooltipster({
		trigger: 'custom',
		onlyOne: false,
		position: 'top'
	});
	$('#loginToggleForm input[type="password"]').tooltipster({
		trigger: 'custom',
		onlyOne: false,
		position: 'bottom'
	});
	...
	$('.menu-item').click(function () {
		...
		$('.menu-toggle').not($($(this).data('target'))).hide(10);
		$($(this).data('target')).slideToggle(500);
		
		$('#loginToggleForm input[type="text"]').tooltipster('hide');
		$('#loginToggleForm input[type="password"]').tooltipster('hide');
		$('#subscr-form input[type="text"]').tooltipster('hide');		
	});
	
	$('.menu-item,.menu-toggle').click(function(e){
		e.stopPropagation();
	});
	
	$(document).click(function() {		
		if( this.class != '.menu-toggle') {
			var $bgs = $('.menu-toggle');
			$bgs.filter(':visible').stop(true, true).slideUp('fast');
			$('.menu-item').removeClass('active');
			$('#loginToggleForm input[type="text"]').tooltipster('hide');
			$('#loginToggleForm input[type="password"]').tooltipster('hide');
			$('#subscr-form input[type="text"]').tooltipster('hide');	
	...
	var CaptchaCallback = function() {
		...
		if ( $('#RecaptchaField1').length ) {
			widgetId1 = grecaptcha.render('RecaptchaField1', {'sitekey' : '6LcCIrgUAAAAAA-uheOFyS5qfPCTVTbe9AOX35A5', 'callback' : headerCapchaLogin});
		}
		if ( $('#RecaptchaField2').length ) {
			widgetId2 = grecaptcha.render('RecaptchaField2', {'sitekey' : '6LcCIrgUAAAAAA-uheOFyS5qfPCTVTbe9AOX35A5', 'callback' : pageCapchaLogin});
		}
		if ( $('#RecaptchaField3').length ) {
			widgetId3 = grecaptcha.render('RecaptchaField3', {'sitekey' : '6LcCIrgUAAAAAA-uheOFyS5qfPCTVTbe9AOX35A5', 'callback' : pageCapchaSignUp});
		}
		if ( $('#RecaptchaField4').length ) {
			widgetId4 = grecaptcha.render('RecaptchaField4', {'sitekey' : '6LcCIrgUAAAAAA-uheOFyS5qfPCTVTbe9AOX35A5', 'callback' : pageCapchaConfirm});
		}
	};
	window.CaptchaCallback = CaptchaCallback;
	var headerCapchaLogin = function(response) {
		$("#hcl_hiddenRecaptcha").val(response);
	};
	var pageCapchaLogin = function(response) {
		$("#pcl_hiddenRecaptcha").val(response);
	};
	var pageCapchaSignUp = function(response) {
		$("#pcsu_hiddenRecaptcha").val(response);
	};
	var pageCapchaConfirm = function(response) {
		$("#pcc_hiddenRecaptcha").val(response);
	};
	
	$('#loginToggleForm').validate({
		ignore: ".ignore",
		rules: {
			auth: {
				required: true,
				remote: {
					url: "/assets/checkUsername.php"
				}
			},
			password: {
				required: true,
				remote: {
					url: "/assets/checkPass.php",
					data: {
						username: function() {
							return $("#auth").val();
						}
			...
			hcl_hiddenRecaptcha: { required: true, },
		},
		messages: {
			auth: {
				required: "Enter username or email!",
				remote: "Invalid username or Email!"
				},
				password: {
	...
	$('#subscr-form input[type="text"]').tooltipster({
		trigger: 'custom',
		onlyOne: false,
		position: 'top'
	});
	...
	$('#subscr-form').validate({
		...
		rules: {
			subscr_mail: {
				...
				email: true,
				remote: {
					url: "/assets/checkSubscrEmail.php"
				}
			}
		},
		messages: {
		...
		submitHandler: function (form) { 
			$('#subscrBtn').attr('disabled','disabled');
			$.ajax({
				url : "/assets/sendSubscr.php",
				type: "POST",
				data : $(form).serialize(),
				success: function( ) {		     
					$('#subscr').html('<div class="compl-ok">Текст пусть дадут.</div>');
				}
			});
	...
	$('#search_fd').on('input', function() {
		var search = $('#search_fd').val();
		if (search.length > 2){
			$.ajax({
				url : "/assets/quickSearch.php",
				type: "POST",
				data : {'search':search},
	...
	$('#search_f').on('input', function() {
		var search = $('#search_f').val();
		if (search.length > 2){
			$.ajax({
				url : "/assets/quickSearch.php",
				type: "POST",
	...
	$(document).on('click','a.logreg',function(){		
